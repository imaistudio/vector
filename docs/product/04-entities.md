...
- GitHub artifacts are evidence by default. Workspace policy may keep evidence manual, notify the owner when terminal evidence needs review, or update state when the individual Work also opts into GitHub completion. Unlinking or suppressing an attachment is durable until a user explicitly links that artifact again; later webhook updates do not recreate unmatched Work or Requests behind the user's back.
